...
export 'package:muvio/data/models/social_models.dart';
...
class SocialCubit extends Cubit<SocialState> {
  ...
  Future<void> _init() async {
    ...
    if (settings.socialUserId == null ||
        settings.socialUserId!.isEmpty ||
        settings.socialUserId == 'null') {
      final String newId =
          'user_${DateTime.now().millisecondsSinceEpoch}_${(DateTime.now().microsecond % 9000).abs()}';
  ...
  Future<void> _syncMonthlyGroups() async {
    ...
    try {
      ...
      for (int i = 0; i < allUsers.length; i += targetSize) {
        final end = (i + targetSize < allUsers.length)
            ? i + targetSize
            : allUsers.length;
      ...
      // 5. Find MY squad
      int mySquadIndex = squads.indexWhere(
        (s) => s.any((u) => u['uid'] == myUid),
      );
  ...
  Future<void> _cleanupOldReports() async {
    ...
    debugPrint(
      '[SOCIAL] Regular maintenance: Cleaning up bug reports older than: $cutoff',
    );
  ...
  Future<void> _processFirestoreMessages(
    ...
    for (var doc in docs) {
      ...
      if (!exists) {
        ...
        if (!isFromMe) {
          final conv =
              existing ?? await _repository.getConversationByRemoteId(cid);
    ...
    if (toAdd.isNotEmpty) {
      ...
      for (var conv in allConvs) {
        final convMsgs = allMsgs
            .where((m) => m.conversationId == conv.remoteId)
            .toList();
        if (convMsgs.isNotEmpty) {
          final latestMsg = convMsgs.reduce(
            (a, b) => a.timestamp.isAfter(b.timestamp) ? a : b,
          );
          if (conv.lastMessage != latestMsg.text ||
      ...
      final updatedConvs = convUpdated
  ...
  Future<void> _loadMockData() async {
    final devSupport =
        await _repository.getConversationByRemoteId('dev_support') ??
              SocialConversation()
          ..remoteId = 'dev_support'
          ..name = 'DEVELOPER (Report Bug)'
          ..isGroup = false
    ...
    final group1 =
        await _repository.getConversationByRemoteId('1') ?? SocialConversation()
          ..remoteId = '1'
    ...
    final group2 =
        await _repository.getConversationByRemoteId('looksmax_3') ??
              SocialConversation()
          ..remoteId = 'looksmax_3'
          ..name = 'LOOKSMAXXING'
          ..isGroup = true
    ...
    final group3 =
        await _repository.getConversationByRemoteId('coach_alpha') ??
              SocialConversation()
    ...
    if (allMsgs.isEmpty) {
      final m1 = SocialMessage()
        ...
        ..text = 'Hello! Welcome to Muvio Social. Cloud sync is now active.'
  ...
  Future<void> sendMessage(String conversationId, String text) async {
    ...
    final allConvs = await _repository.getAllConversations();
    debugPrint(
      '[SOCIAL CUBIT] State updated. Total messages: ${allMessages.length}',
    );
  ...
  Future<void> _handleCoachResponse(
    String conversationId,
    String userText,
  ) async {
    final coachIdStr = conversationId.replaceFirst(
      CoachService.COACH_CONV_ID_PREFIX,
      '',
    );
    ...
    if (!coach.isAi) {
      Future.delayed(const Duration(seconds: 1), () {
        receiveMessage(
          conversationId,
          coach.name,
          "I DON'T CARE! JUST TRAIN!",
          senderId: 'coach_$coachId',
        );
      });
      return;
    }
    ...
    try {
      final response = await locator<CoachService>().generateCoachChatResponse(
        coach,
        userText,
      );
      receiveMessage(
  ...
  void receiveMessage(
    ...
    final existingConv = await _repository.getConversationByRemoteId(
      conversationId,
    );
  ...
  void addFriend(String idOrName) async {
    ...
    if (existing == null) {
      ...
      final newConv = SocialConversation()
        ..remoteId = convId
        ..name =
            idOrName // Will be shown as the peer's ID until they respond
        ..isGroup = false
        ..participantIds = [myId, idOrName]
        ..lastActive = DateTime.now()
        ..isPendingFriendRequest =
            false // I'm the sender - already "accepted" on my side
        ..peerName = idOrName
        ..peerId = idOrName
  ...
  Future<void> _autoEstablishP2PLink(
    String convId,
    String senderName,
    String senderId,
  ) async {
    ...
    if (displayName.isEmpty || displayName.startsWith('user_')) {
      displayName = senderId.length > 12
          ? senderId.substring(senderId.length - 8)
          : senderId;
    }
    ...
    final newConv = SocialConversation()
      ..remoteId = convId
      ..name =
          displayName // Use the REAL sender name!
      ..isGroup = false
      ..participantIds = ['me', senderId]
      ..lastActive = DateTime.now()
      ..isPendingFriendRequest =
          true // INCOMING request - show in pending section
    ...
    emit(state.copyWith(conversations: convs));
    debugPrint(
      '[SOCIAL] Auto-Link (pending) established: $convId from $displayName',
    );
  }
  ...
  void shareContent(
    ...
    final settings = await _bodyRepository.getUserSettings();
    final String? myUid =
        FirebaseAuth.instance.currentUser?.uid ?? settings.socialUserId;
    if (myUid != null) {
      final fs = _firestore;
      if (fs != null) {
  ...
  void _startRestSync() {
    ...
    _restPollingTimer = Timer.periodic(
  ...
  Future<void> _fetchRestUpdates() async {
    try {
      final url = Uri.parse(
        'https://firestore.googleapis.com/v1/projects/fitnesapp-d5c3c/databases/(default)/documents/messages?orderBy=timestamp%20desc&pageSize=50',
      );
      ...
      if (response.statusCode == 200) {
        ...
        for (var doc in documents) {
          ...
          final String cid =
              fields['conversationId']?['stringValue'] ?? 'unknown';
          ...
          final exists = currentMsgs.any(
            (m) =>
                m.remoteId == mid ||
                (m.text == text && m.conversationId == cid),
          );
  ...
  Future<void> _sendRestMessage(String text, String cid) async {
    try {
      final url = Uri.parse(
        'https://firestore.googleapis.com/v1/projects/fitnesapp-d5c3c/databases/(default)/documents/messages',
      );
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
          '[SOCIAL] REST Send Failed: ${response.statusCode} | ${response.body}',
        );
      }
    } catch (e) {
