...
class CoachService {
  ...
  Future<void> checkInactivity(SocialCubit socialCubit) async {
    ...
    final daysSinceWorkout = settings.lastWorkoutDate == null
        ? 3
        : now.difference(settings.lastWorkoutDate!).inDays;
    ...
    if (daysSinceWorkout >= 3) {
      ...
      message = await _generateMessage(
        coach,
        "The user hasn't trained for $daysSinceWorkout days. Give them a short, impactful, and motivating message to get them back to training immediately. No fluff.",
      );
    }
    ...
    if (daysSinceOpen >= 2) {
      message = await _generateMessage(
        coach,
        "User hasn't opened the app for $daysSinceOpen days. They might be getting lazy.",
      );
    }
    ...
    // We can use a random seed or store a 'lastRandomMessageDate' in settings
      ...
      if (rand.nextDouble() < 0.1) {
        message = await _generateMessage(
          coach,
          "Send a funny, random motivational thought or tip in your characteristic style.",
        );
      }
    }
      ...
      final response = await _aiService.getResponse([
        {
          'role': 'system',
          'content':
              '${coach.systemPrompt} Respond to this context: $promptContext. Keep it short (1-2 sentences).',
        },
      ]);
      return response;
    } catch (e) {
  ...
  Future<String> generateCoachChatResponse(
    CoachModel coach,
    String userText,
  ) async {
    return _generateMessage(
      coach,
      "The user just said: '$userText'. Respond in your characteristic style.",
    );
  }
}
