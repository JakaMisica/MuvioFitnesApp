  ...
  static String getMuscleImagePath(MuscleGroup group, [String? subGroup]) {
    ...
    if (subGroup != null && subGroup.isNotEmpty) {
      ...
      if (s.contains('biceps'))
        fileName = 'biceps';
      else if (s.contains('triceps'))
        fileName = 'triceps';
      else if (s.contains('forearm'))
        fileName = 'forearms';
      else if (s.contains('lats'))
        fileName = 'lats';
      else if (s.contains('rhomboid'))
        fileName = 'rhomboids';
      else if (s.contains('lower back'))
        fileName = 'lower_back';
      else if (s.contains('rear delt')) {
        fileName = group == MuscleGroup.back ? 'rear_delt_back' : 'rear_delts';
      } else if (s.contains('upper abs'))
        fileName = 'upper_abs';
      else if (s.contains('lower abs'))
        fileName = 'lower_abs';
      else if (s.contains('oblique'))
        fileName = 'obliques';
      else if (s.contains('quad'))
        fileName = 'quads';
      else if (s.contains('hamstring'))
        fileName = 'hamstrings';
      else if (s.contains('glute'))
        fileName = 'glutes';
      else if (s.contains('calves'))
        fileName = 'calves';
      else if (s.contains('adductor'))
        fileName = 'adductors';
      else if (s.contains('front delt'))
        fileName = 'front_delts';
      else if (s.contains('middle delt') || s.contains('side delt'))
        fileName = 'middle_delts';
      else if (s.contains('upper chest'))
        fileName = 'upper_chest';
      else if (s.contains('middle chest') || s.contains('midle'))
        fileName = 'middle_chest';
      else if (s.contains('lower chest'))
        fileName = 'lower_chest';
      else if (group == MuscleGroup.cardio)
        fileName = 'cardio';
    }
